...
> **状态（0.1.62 落地后）**：这是当时在 `video-support` 分支上的设计稿，落地时有三处口径改了 —— ① 音轨按**原始采样率与声道数**抽出，不是单声道 48 kHz；② 导出**默认不重编码**（`fastcopy`「无损快速」），PRD 里的默认档是 reencode；③ 字幕**这轮只有外挂 SRT**（按成片时间轴重映射），烧录进画面未实施。实际口径以 `CONTEXT.md`、`AGENTS.md` 硬约束 19 与 `docs.md` 的「视频支持（0.1.62 移植）」为准。

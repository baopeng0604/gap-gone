...
8. 如各段音量不一，点 **“响度标准化”**（快捷键 `⇧L`）：按成片 Integrated 响度归一至设置里的目标（默认 `-23 LUFS`，EBU R128 广播口径；另有播客 `-19`、短视频 `-14`），并用前瞻真峰值限幅把峰值压在 `-1 dBTP`（导出 MP3 时 `-1.5 dBTP`）以内。只改增益、不改波形，所以不会削波也不发毛；不满意可点 **“撤销响度”** 回退。

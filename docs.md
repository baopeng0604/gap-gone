...
工具栏第二排末尾的「响度标准化」（快捷键 `L`）基于导出成片的 Integrated 响度（按时间轴保留区间拼出）计算增益，把整段音频统一到 -16 LUFS。它是派生操作，不修改原始音频；仍可用「恢复原始（B）」回退。相比短视频常见的 -14 LUFS，-16 属于更保守、耐听的选择，恰好落在界面「达标」档起点。

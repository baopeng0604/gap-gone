...
* `L`：一键响度标准化，把成片 Integrated 响度归一至 -16 LUFS。
...
工具栏第二排末尾的「响度标准化」（快捷键 `L`）走三段式处理，避免大音量段削波：先对超过 -6 dBFS 的大音量段做轻度 1.5:1 压峰，再基于导出成片 Integrated 响度（按时间轴保留区间拼出）计算增益、统一到 -16 LUFS，最后把真峰值限制到 -1 dBFS 兜底。它是派生操作，不修改原始音频；仍可用「恢复原始（B）」回退。相比短视频常见的 -14 LUFS，-16 属于更保守、耐听的选择，恰好落在界面「达标」档起点。

...
设置页后来陆续加入（2026-09-22 汇总）：**静音阈值**（三档预设 + 手填 dBFS，改完自动清掉旧候选）、**响度目标**（三档预设 + 手填，默认 -23 LUFS，EBU R128 广播口径）。Windows 上首次启动（或保存的设备已拔掉）时录音设备默认选中列表第二个——内置麦通常排第一、用户想用的外接麦在第二位；其它平台仍取第一个，用户显式选过的设备不会被覆盖。转录模型文件齐备时「下载模型」按钮置灰不可点，避免重复下载；只有"转录齐、标点缺"时仍可点。模型目录那条的提交按钮由「保存」改名「应用路径」——它只把输入框里的路径写入配置并重查状态，不点就不生效，叫"保存"容易被误认成保存项目。
...
波形每行表示 10 秒，默认窗口为 1280×820，单行波形宽度为 1000px。
...
* `L`：循环播放开关，点亮即从成片开头开始播放、再按熄灭并停止；`⇧L`：一键响度标准化，把成片 Integrated 响度归一至设置里的目标（默认 -23 LUFS）。
...
**切除区间怎么跳过**：媒体元素没法"按区间拼接"，改成 rAF 里发现 `currentTime` 落进 EditTimeline 区间就 seek 到区间末尾，`seeking` 期间不重复下发。精度从样本级降到约一帧（16 ms），实际影响极小——最短的切除区间也有 0.2 s（静音检测）或上百毫秒（手拖）。顺带简化了一处：点波形跳转原来要停播重建音源，现在只改 `currentTime`。
...
目标响度在设置里可选或手填：广播 -23（默认，EBU R128 标准；美国 ATSC A/85 为 -24 LKFS）、播客 -19、短视频 -14，手动范围 -30 ~ -6。注意口径：-23 按立体声口径测得，单声道节目想严格对齐广播口径应取 -26；-19 是 Apple Podcasts 对**单声道**节目的规范值（立体声 -16，差 3 dB 只是 BS.1770 单双声道口径不同）。真峰值上限按导出格式分档：WAV 按播客规范 -1 dBTP，MP3 因为有损编码还会过冲约 0.5 dB，压到 -1.5 dBTP。

完成后播报响度前后值、真峰值与限幅衰减量；限幅衰减超过 6 dB 会提示"素材动态偏大，可考虑先做压缩"。处理前留一份缓冲快照，工具栏会出现「撤销响度」按钮（Ctrl+Z 只管区间编辑，回不到这一步）；降噪或换文件后快照失效，避免回退时把降噪成果一起吞掉。它是派生操作，不修改原始音频，「恢复原始（B）」仍然有效。
...
点击工具栏「转录文字」（快捷键 `T`）开始：首次使用会先下载模型（约 230MB，显示下载进度），之后转录全程本地完成；首次转录还会顺带下载标点恢复模型（约 75MB，一次性，失败自动降级为无标点输出）。模型目录可在「设置」中自定义（默认 `~/models/sense-voice`，旁边有「打开目录」按钮直达；0.1.6 起默认目录从应用数据目录改到用户主目录，旧位置已有模型的需手动迁移）；「设置」中的「录音确定编辑后自动转录」默认开启。转录结果以两种形态呈现：**每行波形下方的逐字词带**（字按时间戳与波形横向对齐，正在播放的字高亮，被切除区间覆盖的字划线置灰，点击对应位置即跳转——词带是"瞄准镜"，切除/恢复仍用拖拽工具画区间）和**波形下方的句子面板**（点击句子跳转，播放句高亮跟随）。句子文本自动补全标点；`.srt` 字幕按标点分句（多句长条拆分、碎句合并），`.txt` 文字稿自动分句分段（停顿换段 + 句末收段），两者均可导出文件或一键复制到剪贴板。转录锚定原始完整时间轴，切除/恢复不需要重跑。

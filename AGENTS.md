...
6. **DfTract 不是 Send（内含 Rc），不能放进 Tauri State；也绝不要缓存复用 —— 每个降噪任务都要新建模型**。模型只在 `gap-gone-denoise` 工作线程里创建与使用，命令只投递 `DenoiseJob`。**`init()` + `DFState::reset()` + `init_norm_states()` 这套「重置」并不能复位模型内部的滚动缓冲**（0.1.59 实测）：复用同一个模型连跑三次同一段音频，输出相对输入分别晚 50 / 100 / 150 ms —— 每次多漂 `df_order` 帧；而每次重建模型，三次结果逐采样一致。漂移对整段降噪只是整体平移（听不出来），但**选区降噪会变成内容错位**（交叉淡化只盖得住电平跳变），波形/转录与音频的对齐也会一起错。所以 `denoise_worker` 里不再有 `Option<DfTract>` 缓存，模型建在 `run_denoise_job` 内部；代价是每个任务多花一两秒加载模型，可以接受。SenseVoice 转录同理常驻 `gap-gone-transcribe` 线程。
...
14. **DeepFilterNet 的引擎配置与运行时参数都必须对齐官方口径，别沿用库默认**。**模型用低延迟变体（`default-model-ll`，见硬约束 18）**。模型构造要用 `RuntimeParams::default_with_ch(1).with_thresholds(-15.0, 35.0, 35.0).with_mask_reduce(ReduceMask::MAX)`（`denoise_runtime_params()`），**不要**直接传 `default_with_ch(1)`（库默认是 −10 / 30 / 20 且 `reduce_mask: MEAN`）。crate 的 `apply_stages` 拿这三个阈值按帧的 local SNR 决定做多少处理：lsnr > `max_db_erb_thresh` → **整帧原封不动**（连增益都不算）；`max_db_df_thresh` < lsnr ≤ `max_db_erb_thresh` → 只做 ERB 掩蔽、**跳过 DeepFilter 第二级**；lsnr < `min_db_thresh` → 全零掩蔽。库默认的 df 阈值只有 20 dB，恰好把口播里「有语音、底下压着一层底噪」的帧（local SNR 约 20 ~ 35 dB）划进「跳过 DF」甚至「整帧不动」——**降噪在最需要它的地方被提前关掉**，实测听感就是「一说话噪声更明显」（0.1.53 用户实测后定位，官方 `enhance_wav.rs` 与 `capi.rs` 都是 −15 / 35 / 35）。这三个阈值只在 `DfTract::new` 时生效，**没有运行时 setter**；运行时可改的只有 `set_atten_lim`（档位）与 `set_pf_beta`（post-filter）。**`atten_lim` 不是「每频段最多压 N dB」**，而是「把原始含噪频谱按 `10^(−N/20)` 的比例全频段掺回输出」——轻档 12 dB 掺回 25%、中档 24 dB 掺回 6.3%、强档 36 dB 掺回 1.6%（原来给的是 100 dB，等于 `None`／完全不掺回，0.1.56 起封顶在 36 dB。**0.1.60 实测推翻了这个封顶的理由**：同一条 SNR 18.8 dB 的口播素材上把 `atten_lim` 从 24 → 36 → 48 → 100（`None`）逐档跑，停顿段电平只从 −58.3 变到 −59.0 dBFS，逐块 5–16 kHz 的差 ≤ 0.3 dB —— **24 dB 以上这整段阶梯是空转的**；「100 dB 会在低信噪比素材上挖谱洞、压出破音」那个听感同样出自兜底算法时期（见硬约束 15/18）。这条阶梯的实际有效范围只有「轻 → 中」这一档），改档位前先想清这一点。**必须补偿 `process` 的算法延迟，而且公式要算上 `df_order`**：`delay = fft_size − hop_size + (df_order + lookahead) × hop_size`（本模型 = 960 − 480 + 5 × 480 = **2880 采样 = 60 ms**）。**官方 `enhance_wav` 的 `fft_size − hop_size + lookahead × hop_size` 漏了 `df_order` 这一项**（它的 `--compensate-delay` 同样漏），照抄会让整段输出晚 5 个 hop（实测 2400 采样 = 50 ms）。原因是 crate 的 `process` 把输出取自滚动缓冲里**最旧**的那一帧，而缓冲长度是 `(df_order + conv_lookahead)` 帧。**判据**：用幅度包络（对基音周期免疫）做互相关，补对了峰值应落在 **lag 0** 且 `ρ(0) ≥ 0.99`；补不够时 lag 会是 `hop_size` 的整数倍（原始波形做相关会看不出，因为男声 100 Hz 的周期正好等于 1 个 hop）。`diagnostic_denoise_alignment`（`GAP_GONE_DENOISE_TEST_WAV=<wav>`）会直接打印这两个数。**不补的话整段降噪会整体滞后**（与视频/字幕对不齐），**选区降噪直接错位**（交叉淡化只能盖住电平跳变，盖不住内容错位）。post-filter **保持关闭（`set_pf_beta(0.0)`）**。它会在「掩蔽不确定」的频点上再压一层，官方 CLI 默认 0.02，但**代价是语音失真**：低信噪比素材上「不确定」的频点特别多，0.1.54 试开 0.02 的结果是残余噪声没消掉、人声先发毛/破音，0.1.55 回退（用户实测）。另外注意链上顺序：post-filter 在「掺回原始噪声」之前跑（见 crate 的 `process`），所以它本来也压不掉 `atten_lim` 掺回的那部分 —— 那部分只能靠调档位来减（强档不掺）。

15. **降噪的调参有天花板，别指望旋钮能救回低信噪比素材**。DFN 的取舍就一条：`atten_lim` 大 → 留残余噪声（中档 24 dB 掺回 6.3%），`atten_lim` 往满里调到 `None` → 模型全力抑制，低信噪比素材上可能吐出谱洞与帧间抖动。**注意：0.1.53~0.1.55 那批「中档留残余、强档人声破音」的听感全部来自兜底算法，不是 DeepFilterNet**（见硬约束 18），所以「该素材已经到头了」这个结论必须在引擎真正跑起来之后重新判定 —— 别再拿那批听感当证据去否参数口径。**0.1.60 已完成重判**（`diagnostic_denoise_alignment`，48 kHz 单声道 26.99 s、SNR 18.8 dB、块电平 P10 −46.76 / 中位数 −27.99 的真实口播素材，前 15 s）：轻 12 / 中 24 / 强 36 / 极限 48 / 不掺回 100 五档下，停顿段（固定块索引）依次 −55.81 / −57.75 / −58.04 / −58.9 / −59.0 dBFS，语音段只降 0.4 ~ 0.5 dB。三条结论：① 抑制在 **≈ 9.5 dB 处饱和**，这是**内容限制**、不是参数限制；② 中 → 强只多 0.3 dB，24 dB 以上无区分度，所谓「轻/中/强」在这类素材上实际只有两态；③ **延迟补偿得到独立确认**（五档全部 lag 0 采样、ρ(0) 0.990 ~ 0.994），每任务重建模型的输出逐采样可复现（自检里已从打印升级为断言）。所以降噪侧不要再指望「再调一档试试」：要更干净只能动录音端（离麦更近、把底噪压下去）或链路端（别在压缩里把信噪比吃掉）。

16. **去嘶声（7 kHz 以上高频压制）已在 0.1.58 移除，别再无条件加回来**。它当初的理由是「DeepFilterNet 的 DeepFilter 只覆盖约 5 kHz 以下，人声段那层嘶嘶是从高频漏出来的」——**但支撑它的证据（中档留残余、强档人声破音的听感）全部来自兜底算法，不是 DeepFilterNet**（见硬约束 18）。引擎真正跑起来之后再叠一层最多 10 dB 的高频衰减，只会把语音削「闷」、加重「发重」感，属于在没有证据的前提下动音色。要加回来必须先有**在 DFN 正常工作时**测到的残余高频噪声证据，并从更温和的深度（≤ 4 dB）起步。**0.1.60 补上了这份证据，而它的形状推翻了原假设**：按「相对输入」量全部 150 个 100 ms 块的 5–16 kHz 电平变化，中位数是 −3.0 dB（多数块在降，停顿块最高降 19 dB），但**有 46 ~ 50 个块（约 1/3）高频反而升高，最大 +24 dB**，而且**在完全不掺回（`atten_lim = None`）时依然有 +19.8 dB** —— 所以它不是「DFN 只覆盖约 5 kHz 以下、高频漏出来的稳定嘶声」，而是**偶发、窄时（约 100 ms）、宽带的模型伪影**。一个静态的 7 kHz shelving 治不了偶发爆点，只会把语音整体削闷 —— 这反过来支持 0.1.58 的删除决定。要再动，得先分清伪影的机制（帧率门控/音乐噪声 vs 分块边界），并针对**离群块**而不是全段。另记一条方法论：**分带电平不能只对片段求和**，会被单个爆点主导（最初把 15 个停顿块的功率求和得出「整体 +9 dB」，拆成逐块才发现是其中 2 个块贡献的，其余 13 个都在降）。原实现见 git 历史（`applyDeHiss`，0.1.56 引入）。
...
2. **降噪会在个别块上凭空抬高 5–16 kHz（偶发伪影）**。实测全部 150 个 100 ms 块里约 1/3 的高频反而升高、最大 +24 dB；**与 `atten_lim` 无关**（开到完全不掺回时依然 +19.8 dB），所以不是掺回造成的；机制（模型/合成侧 vs 分块边界 vs 延迟补偿的尾部补零）未查。它是「降噪后偶发嘶声/毛刺」的候选来源，而静态高频 shelving 治不了它 —— 要动就针对**离群块**。复现走 `diagnostic_denoise_alignment` + 按 100 ms 块逐块比对 5–16 kHz。**影响面（2026-09-26 实测）**：**宽带电平上看不见它**（伪影块的宽带变化只有 −0.6 ~ −6.0 dB，多数是变安静），所以电平表、LUFS、响度归一、真峰值限幅、静音检测都不受影响、也不会触发任何一层的判定；它改的是**频谱形状**（输入里 5–16 kHz 比宽带低 27 ~ 40 dB，输出里变成低 1 ~ 24 dB），而且**语音块也会中招**（输出 5–16 kHz 比语音自身高频中位数高约 10 dB）。**所以下一步不是查机制，而是先用耳朵确认存在性**：轻档降噪后跳到 6.5 s 与 9.3 s 附近反复听。数据与判读见 `docs.md` 同一节。
